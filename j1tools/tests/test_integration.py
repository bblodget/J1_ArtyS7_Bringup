...
def test_assembly_with_constants():
    ...
    try:
                    ...
                    if isinstance(result, list):
                        for i, instr in enumerate(result):
                            if instr:
                                ...
                                assembler.instruction_metadata[
                                    len(assembler.instruction_metadata)
                                ] = instr
                    else:
                        # Add to instruction_metadata with a unique address
                        assembler.instruction_metadata[
        ...
        assert "MAX_VALUE" in assembler.directives.constants
        assert assembler.directives.constants["MAX_VALUE"] == 100
        assert "MIN_VALUE" in assembler.directives.constants
        assert assembler.directives.constants["MIN_VALUE"] == 10
        assert "MULTIPLIER" in assembler.directives.constants
        assert assembler.directives.constants["MULTIPLIER"] == 2
        assert "ASCII_A" in assembler.directives.constants
        assert assembler.directives.constants["ASCII_A"] == 65
        assert "SOME_HEX" in assembler.directives.constants
        assert assembler.directives.constants["SOME_HEX"] == 0xBEEF
        assert "TWICE_MAX" in assembler.directives.constants
        assert assembler.directives.constants["TWICE_MAX"] == 100
        assert "OFFSET" in assembler.directives.constants
        assert assembler.directives.constants["OFFSET"] == 20

        # Check that the main label is defined
        assert "main" in assembler.labels

        # Check that ENTRY_POINT references the main label
        assert "ENTRY_POINT" in assembler.directives.constants
        assert assembler.directives.constants["ENTRY_POINT"] == assembler.labels["main"]
