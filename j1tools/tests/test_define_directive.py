...
def test_define_decimal_constant():
    ...
    assert 'ANSWER' in assembler.constants
    assert assembler.constants['ANSWER'] == 42
...
def test_define_hex_constant():
    ...
    # Check constant is defined with correct value
    assert 'ANSWER_HEX' in assembler.constants
    assert assembler.constants['ANSWER_HEX'] == 42
...
def test_define_char_constant():
    ...
    assert assembler.constants['ASCII_A'] == 65  # ASCII value for 'A'
...
def test_reference_existing_constant():
    ...
    assert 'ANSWER' in assembler.constants
    assert assembler.constants['ANSWER'] == 42
    assert 'ALSO_ANSWER' in assembler.constants
    assert assembler.constants['ALSO_ANSWER'] == 42
...
def test_reference_undefined_constant():
    ...
    with pytest.raises(ValueError, match="Referenced constant or label 'UNDEFINED' is not defined"):
        assembler.define_directive(items)
...
def test_expression_evaluation():
    ...
    assembler.constants["A"] = 10
    assembler.constants["B"] = 5
